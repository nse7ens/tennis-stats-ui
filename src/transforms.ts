...
export function computeBestRound(results: RawResult[]): { score: number; label: string } {
  const scores: number[] = [];
...
export function transformDisc(d: RawDisc): UIDisc {
  return {
...
export function fmtSetTuple(s: unknown): string | null {
...
export function transformRecent(m: RawRecentMatch): UIRecentMatch {
  const ms = m.m_score;
...
export function transformUpcoming(u: RawUpcomingMatch): UIUpcomingMatch {
...
export function transform(raw: RawPlayerData): UIPlayerData {
  return {
